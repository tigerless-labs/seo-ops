...
def verify_checklist_sync():
    ...
    for cid in sorted(set(d) - set(c), key=lambda s: int(s[1:])):
        print(f"⚠️  漂移:checklist 有 {cid}(优先级 {d[cid][2]}),脚本未实现", flush=True)
    for cid in sorted(set(c) - set(d), key=lambda s: int(s[1:])):
        print(f"⚠️  漂移:脚本有 {cid},checklist 已无此条目", flush=True)
    for cid in sorted(set(d) & set(c), key=lambda s: int(s[1:])):
        if d[cid][2] != c[cid][2]:
            print(f"⚠️  漂移:{cid} 优先级 checklist={d[cid][2]} / 脚本={c[cid][2]}", flush=True)
        elif d[cid][0] != c[cid][0]:
            print(f"⚠️  漂移:{cid} 所属节 checklist=第{d[cid][0]+1}节 / 脚本=第{c[cid][0]+1}节", flush=True)
...
def load_sites(args):
    if args.target or CFG.TARGET:
        ...
        return [{"id": p.netloc.replace(":", "_"), "origin": t,
                 ...
                 "sitemap": None}]
    data = yaml.safe_load((ROOT / "sites.yaml").read_text())["sites"]
    sites = []
    for s in data:
