...
```
seo/
  _site.yaml                 ← 站级,全站一份(见「一、站级」)
  guide-f1-to-h1b.yaml       ← 对应 /guide/f1-to-h1b
  sort-f1.yaml               ← 对应 /sort/f1
  sort-h1b.yaml
```

- **一页一个文件**,文件名 = URL 路径去掉开头的 `/`、其余 `/` 换成 `-`。
  这样文件名和 URL 一一对应,不用维护一张对照表,谁都能一眼认出这份是哪页的
- 页面 doc、layout 设计稿**照旧**,位置和写法都不动;SEO 文件与它们**并列**,
  在页面 doc 顶部留一行指过去就行:`SEO: seo/guide-f1-to-h1b.yaml`
- 分开的理由:这些字段是**被逐条查的输入**(checker + 人审),而设计稿是给人看的。
  混在一起,查的人要从文案中间去找,写的人改版式时又容易顺手动了它们
...
## 一、站级 `seo/_site.yaml`(**全站一份,交一次**)

T1 / T2 不是每页填的 —— 但**先有它们**:品牌名没定,每页的 title 和 JSON-LD 里的
实体就各写各的。所以先交这一份,后面每页只管自己那个文件。
...
## 二、每页 `seo/<page>.yaml`(**每页必交**)

下面整份就是一个页面文件的全文,复制过去改值即可。
...
| pSEO 批量场景页 | **与人写的页一视同仁** —— 同样一页一个 SEO 文件、title/desc 唯一、alt 逐张写。(说明:进 llms.txt 时以 hub 页代表,几百条场景页不逐页罗列) | T5 · T14 · T2 |
| 作者 / 团队页 | 每人实名 + 头衔 + 可验证资质,与文章页 byline 用**同一套写法** | T12 |
| 分类 / 列表 / 对比页 | 清单项要在页面上可见、可点;别只给一张对比图。(说明:图里的内容 AI 读不到) | T14 · T7 |
| 首页 | 品牌名写法以 `_site.yaml` 为准,不临时改口径 | T1 |
