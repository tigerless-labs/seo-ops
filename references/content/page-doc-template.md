...
> 结构:**0 站级(全站一份,交一次)· A 头部块(每页必交)· B 图片 alt(唯一写在正文里的)·
> C 条件项 · D 特殊页型参照 · E 正文与 layout(你们原有的)**。

## 怎么用

1. **0 节全站交一次**(住站级配置表,不进页面 doc);**A 节起整份复制到页面 doc 顶部**,
   一页一份。**SEO 供给字段与正文分开写** ——
...
## 0. 站级供给(**全站一份,交一次**;页面 doc 里不用带这节)

T1 / T2 不是每页填的,它们住站级配置表 —— 但**先有它们**:品牌名没定,每页的 title
和 JSON-LD 里的实体就各写各的。所以放在第一节,交完一次,后面每页只管 A 节开始。
...
【此处开始:按 layout 分段写文案 —— 与现有 APSU Website 页面 doc 相同的写法。
 ...
 全部照旧放这里。本模板只负责上面 0 与 A–C 节的 SEO 供给字段,不改变这部分的任何写法。】
```
...
|---|---|---|
| ☐ | 占位符 | 搜 `【`,应该一个都搜不到 |
| ☐ | 0 节 | 站级那份已经交过、且是最新的(改过名 / 换过 logo / 开过新官方号就要更新) |
