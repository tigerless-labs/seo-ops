...
大纲交**层级结构 + 每节第一句话的意思**,不是标题清单:

```markdown
## 【F1 学生必须买医疗保险吗?】
   → 首段结论:【大多数学校强制要求,且必须满足 waiver 条款;不是联邦法律强制。】

## 【学校的 waiver 要求一般看哪几项?】
   → 首段结论:【四项——年度上限、自付额、遣返条款、承保期覆盖整个学期。】

### 【自付额(deductible)怎么算合格】
   → 首段结论:【多数学校要求 ≤$500……】
```
...
- **每节第一段就是结论**,论证放后面
- FAQ 写成独立问答块(供 FAQPage JSON-LD 取值)
...
| ☐ | T6 · T7 | h1 一个;大纲不跳级、每节结论前置 |
| ☐ | T14 | 每张内容图都有 alt,装饰图标了「装饰」 |
