...
**脚本只产出结论与证据,不解释** —— 它是纯确定性的、零 LLM,那是这份报告能当验收依据、
能拿去跟施工方争议的全部理由。解释由你在报告**生成之后**补进去:

1. 打开 `~/Documents/seo-ops/out/report-<site>-<date>.md`,给三张表各加一列 **`解释`**
   (表头与分隔行都要加,否则 markdown 表格会错位)。
2. **非 pass 的行**填两句:**这为什么是问题**(后果落到收录 / 被引用上)+ **怎么改**。
   pass 的行填 `—`。
3. **每一格都先读 `references/checklist/references/C<N>.md`,解释与修法都出自那篇** ——
   「为什么是问题」取自它的 **`## 介绍`**,「怎么改」取自它的 **`## 实现指导`**。
   **不要只看证据推断**:证据只告诉你「哪几页不合格」,说不出这条为什么存在、
   正解是什么、权威依据在哪。26 篇都有这两节,凭印象写出来的解释常常是对的废话,
   偶尔是错的 —— 而用户会拿它去跟施工方对话。
4. **每格控制在 80 字以内**,并在末尾附 `[C<N>](references/checklist/references/C<N>.md)`
   让用户能往下翻。证据列已经因为过长撑爆过表格,别再撑一次。
5. 写回原文件。**重跑 checker 会覆盖掉这一列** —— 要长期留存就另存一份。

补完之后再口头给结论:**按 P0 → P1 → P2 讲,不按红项数量**(一条 P0 比五条 P2 重要);
**N.A. 必须点出来**并说清缺什么导致的(缺 key / 缺声明 / 被限流)——「没测」不是「没事」,
用户会把看着全绿的报告读成「没问题」;**人审项(C21/C22)列出来提醒人过,不替人下结论**;
报告顶部若有 🚦 限流或 ⚠️ 抓取失败告警,**先说这次结论不完整**,再讲发现。
