...
报告里每行都带一列 **说明**,链到该条的正本 `references/checklist/references/C<N>.md`
—— 判定标准、常见错法、权威依据、怎么改都在那篇。**讲解时先读它**,
「为什么是问题」取自 `## 介绍`,「怎么改」取自 `## 实现指导`。

**不要只看证据推断**:证据只告诉你「哪几页不合格」,说不出这条为什么存在、正解是什么。
凭印象写的解释常常是对的废话,偶尔是错的 —— 而用户会拿它去跟施工方对话。

给结论时:

- **按 P0 → P1 → P2 讲,不按红项数量** —— 一条 P0 比五条 P2 重要。
- **N.A. 必须点出来**,并说清是缺什么导致的(缺 key / 缺声明 / 被限流 / 没爬全)。
  「没测」不是「没事」,而用户会把看着全绿的报告读成「没问题」。
- **人审项(C21 / C22)列出来提醒人过**,不要替人下结论。
