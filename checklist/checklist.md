...
(2026-08-24 修订:三节改人审;
**节内按优先级 P0→P1→P2 排序后重新连续编号(C1–C22,无空号)**——
此前的编号体系及其间的并入/退役记录整体见 git 历史;自此冻结,只顺延。)
(2026-08-25 修订:对照四个开源 SEO/GEO skill 仓做覆盖审计,补六处缺口——
能并的并进旧条目(C2 分片规模 / C6 死链 / C8 header canonical / C18 alt / C19 og:type),
判定动作或优先级不同的新开三条(C23 noindex / C24 viewport / C25 mixed content)。)
...
> 执行层「checker」= 手动运行的检查脚本(tools/check);「站点 CI」= 各 repo 工程师侧。
