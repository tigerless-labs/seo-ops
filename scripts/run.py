...
CHECKS = [
    ("一、站级(每站一次)", [
        ("C1", "P0", "robots.txt 放行全部 AI 爬虫"),
        ("C2", "P0", "sitemap 打得开、条目都活着、更新时间是真的"),
        ("C3", "P0", "同一个页面只有一个网址(www/裸域、http/https、尾斜杠都归到同一个)"),
        ("C26", "P0", "语言版本各有固定网址,不按浏览器语言自动跳"),
        ("C4", "P1", "真实用户的加载 / 响应 / 视觉稳定达标(Core Web Vitals)"),
        ("C5", "P1", "IndexNow 的验证文件放在站根且打得开"),
        ("C6", "P1", "页面上的站内链接不指向坏页(404 / 403 / 5xx)"),
        ("C7", "P2", "站根有 llms.txt —— 给 AI 引擎的站点目录"),
    ]),
    ("二、每收录页", [
        ("C8", "P0", "每页声明自己的规范网址,而且只声明一次"),
        ("C9", "P0", "不执行 JS 也能拿到完整正文"),
        ("C10", "P0", "缓存下来的 HTML 里没有任何因人而异的内容"),
        ("C23", "P0", "要收录的页没有被 noindex 挡住"),
        ("C11", "P1", "每页的标题与摘要各不相同,且都不超长"),
        ("C12", "P1", "结构化数据(JSON-LD)在、必填字段齐、没有已失效的类型"),
        ("C13", "P1", "没有「返回 200 但其实是空页或错误页」"),
        ("C14", "P1", "没有会把整页藏起来的第三方脚本(爬虫可能只拿到空白)"),
        ("C16", "P1", "允许搜索结果展示完整摘要与大图"),
        ("C24", "P1", "有移动端 viewport 声明"),
        ("C17", "P2", "标题层级是一棵树:一个 h1,往下逐级不跳"),
        ("C18", "P2", "每张图都写了宽高和 alt"),
        ("C19", "P2", "分享到社交平台能出正确的预览卡(Open Graph)"),
        ("C20", "P2", "不绕路:任何网址最多跳一次就到最终页"),
        ("C15", "P2", "收录页不是每次请求现渲染(SSR 要有 CDN 缓存)"),
        ("C25", "P2", "HTTPS 页里没有走 http:// 的图片或脚本"),
    ]),
    ("三、条件项(人审项;flag / 站点配置触发)", [
        ("C21", "P0", "YMYL 内容有作者、审核与权威引用"),
        ("C22", "P1", "语言版本之间用 hreflang 双向指认"),
    ]),
]
