...
CHECKS = [
    ("一、站级(每站一次)", [
        ("C1", "P0", "robots.txt 放行全部 AI 爬虫 UA"),
        ("C2", "P0", "sitemap 可达、条目无 4xx/5xx、lastmod 真实、单份 ≤5 万条"),
        ("C3", "P0", "网址归一:www/裸域、http/https、尾斜杠 301 到同一个 canonical host"),
        ("C26", "P0", "不按 Accept-Language 分流:语言版本各有固定 URL,不自动跳"),
        ("C4", "P1", "Core Web Vitals 达标:LCP<2.5s / INP<200ms / CLS<0.1"),
        ("C5", "P1", "IndexNow key 文件在站根可达"),
        ("C6", "P1", "站内出链无 4xx/5xx:页面上的链接不指向坏页"),
        ("C7", "P2", "llms.txt 存在:站根给 AI 引擎的站点目录"),
    ]),
    ("二、每收录页", [
        ("C8", "P0", "自引用 canonical:每页指向自己的规范 URL,HTML 与 header 不冲突"),
        ("C9", "P0", "服务端直出完整正文:不执行 JS 也拿得到(禁 CSR 空壳)"),
        ("C10", "P0", "缓存 HTML 为无个性化公共版:不含因人而异的内容"),
        ("C23", "P0", "收录页无 noindex:meta robots 与 X-Robots-Tag 都没挡"),
        ("C11", "P1", "title / description 每页唯一且不超长(≤60 / ≤150)"),
        ("C12", "P1", "JSON-LD:块可解析、必填字段齐、无已失效类型"),
        ("C13", "P1", "无 soft 404:不返回 200 却给空页或错误页(下架走 301/410)"),
        ("C14", "P1", "无 anti-flicker 脚本把整页藏起来(爬虫可能只拿到空白)"),
        ("C16", "P1", "snippet 控制:max-snippet:-1 + max-image-preview:large"),
        ("C24", "P1", "viewport meta 含 width=device-width"),
        ("C17", "P2", "标题层级:恰好一个 h1,h2→h3 逐级不跳级"),
        ("C18", "P2", "每张 img 有显式 width/height + alt 属性"),
        ("C19", "P2", "Open Graph 全套 + twitter:card:社交预览卡完整且取值正确"),
        ("C20", "P2", "重定向 hop ≤1:不叠跳转链"),
        ("C15", "P2", "渲染成本不按请求计:SSR 需 CDN 缓存(s-maxage + SWR),SSG 免"),
        ("C25", "P2", "无 mixed content:HTTPS 页的子资源不走 http://"),
    ]),
    ("三、条件项(人审项;flag / 站点配置触发)", [
        ("C21", "P0", "YMYL 信任块:作者、审核标注、权威引用"),
        ("C22", "P1", "hreflang 双向互指 + x-default"),
    ]),
]
